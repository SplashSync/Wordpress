...
class WordPress_Plugin_Template_Admin_API
{
     ...
     * @param array  $postTypes    Post types to which this metabox applies
    ...
    public function add_meta_box($id = '', $title = '', $postTypes = array(), $context = 'advanced', $priority = 'default', $callbackArgs = null)
    {
        ...
        foreach ($postTypes as $postType) {
            add_meta_box($id, $title, array( $this, 'meta_box_content' ), $postType, $context, $priority, $callbackArgs);
        }
    }
     ...
     */
    public function save_meta_boxes($postId = 0)
    {
        if (! $postId) {
            return;
        }
        ...
        foreach ($fields as $field) {
            if (isset($_REQUEST[ $field['id'] ])) {
                update_post_meta($postId, $field['id'], $this->validate_field($_REQUEST[ $field['id'] ], $field['type']));
            } else {
